...
class SIFTOrientationLayer(nn.Module):
    ...
    def get_orientation_bin_weights(self) -> torch.nn.Parameter():
        ...
        # TODO: YOUR CODE HERE                                                #
        #######################################################################
        weights = angles_to_vectors_2d_pytorch(torch.from_numpy(np.array([np.pi / 8, 3 * np.pi / 8,
                                                    5 * np.pi / 8, 7 * np.pi / 8, 9 * np.pi / 8, 11 * np.pi / 8,
                                                    13 * np.pi / 8, 15 * np.pi / 8,])).float()) # weights.shape [8, 2]
...
def get_siftnet_features(img_bw: torch.Tensor, x: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    #     x_grid, y_grid = get_sift_subgrid_coords(y[i], x[i])
    #     vector = np.empty((1, 0))
    #     for j in range(x_grid.shape[0]):
    #         vector = np.append(vector,sift[0,:,x_grid[j], y_grid[j]]) #.detach().numpy())
    #     vector = np.where(vector < 0, 0, vector)
    #     norm = np.linalg.norm(vector)
    #     vector = vector.flatten()
    #     tmp = vector / norm
    #     fvs = np.append(fvs, tmp ** 0.9)
    ...
    # #sift = sift.squeeze(0)
    # #print(sift.shape)
    # sift = np.reshape(sift, (1, sift.shape[2], sift.shape[3], sift.shape[1])) # 1 17 17 8
    # print(sift.shape)
    # fvs = np.empty((1, 0))
    # for i in range(x.shape[0]): # 4
    #     x_grid, y_grid = get_sift_subgrid_coords(x[i], y[i])
    #     vector = np.empty((1, 0))
    #     for j in range(x_grid.shape[0]): # 16
    #         vector = np.append(vector, sift[0, y_grid[j], x_grid[j], :])
    #     vector = vector.flatten()
    #     norm = np.linalg.norm(vector)
    #     vector = vector / norm
    #     fvs = np.append(fvs, vector ** 0.9)
    # fvs = np.reshape(fvs, (x.shape[0], 128))
    # print(fvs)
    # print(fvs.shape)
    # print('3th=================right')
    ...
    for i in range(x.shape[0]):
        ...
        vector[i] = vector[i] / np.sqrt(np.sum(np.multiply(vector[i], vector[i])))
