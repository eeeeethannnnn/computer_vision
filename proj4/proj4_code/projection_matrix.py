...
from scipy.linalg import rq as rq
...
def objective_func(x, **kwargs):
    ...
    diff = (projection(x, points_3d) - points_2d).flatten()
    ############################################################################
    #                             END OF YOUR CODE
    ############################################################################
...
def estimate_camera_matrix(pts2d: np.ndarray, 
    ...
    ls = least_squares(objective_func, P, method='lm', verbose=0, max_nfev=50000, kwargs=kwargs, ftol=1e-08,)
