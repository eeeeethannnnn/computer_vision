...
def perspective_n_points(initial_box_points_3d, box_points_2d, intrinsic_matrix):
    ...
    # TODO: YOUR CODE HERE
    ############################################################################
    # print(initial_box_points_3d)
    # print(box_points_2d)
    # print(intrinsic_matrix)
    retval, rvecs, tvecs = cv2.solvePnP(initial_box_points_3d, box_points_2d, intrinsic_matrix, np.zeros((8, 1)))
    wRc_T, jacobian = cv2.Rodrigues(rvecs)
    camera_center = - np.matmul(wRc_T.transpose(), tvecs)
    P = np.matmul(intrinsic_matrix, np.concatenate((wRc_T, tvecs), axis=1))
    ############################################################################
    #                             END OF YOUR CODE
    ############################################################################
