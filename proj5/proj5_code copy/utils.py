...
def projection_2d_to_3d(P, depth, pose2d):
    ...
    # TODO: YOUR CODE HERE
    ############################################################################
    pose2d = np.concatenate((pose2d, np.ones((pose2d.shape[0],1))), axis=1)
    pose2d = pose2d * depth
    pose2d = pose2d - P[:, 3]
    pose3d = np.matmul(np.linalg.inv(P[:,0:3]), np.transpose(pose2d))
    pose3d = np.transpose(pose3d)
    ############################################################################
    #                             END OF YOUR CODE
    ############################################################################
    return pose3d
...
def get_world_vertices(width, height, depth):
    ...
    # TODO: YOUR CODE 
    ############################################################################
    for i in range(vertices_world.shape[0]):
        if i % 2 == 1:
            vertices_world[i, 0] = width
        if i > 3:
            vertices_world[i, 1] = height
        if i == 2 or i == 3 or i == 6 or i == 7:
            vertices_world[i, 2] = depth
    ############################################################################
    #                             END OF YOUR CODE
    ############################################################################
